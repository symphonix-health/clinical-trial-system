...
async def test_get_subject_by_external_id_no_identifier(
...
async def test_process_appointment_confirmed_by_screening_id(
    db_session: AsyncSession,
) -> None:
    ...
    result = await inbound_processors.process_appointment_confirmed(
        db_session,
        {
            "screening_id": domain["subject"].screening_id,
...
async def test_process_lab_result_subject_not_found(
    db_session: AsyncSession,
) -> None:
...
async def test_process_imaging_result_subject_not_found(
    db_session: AsyncSession,
) -> None:
...
async def test_process_dispense_event_product_not_found(
    db_session: AsyncSession,
) -> None:
    domain = await _seed_minimal_domain(db_session)
...
async def test_process_dispense_event_success(
    db_session: AsyncSession,
) -> None:
    domain = await _seed_minimal_domain(db_session)
    result = await inbound_processors.process_dispense_event(
...
async def test_process_agent_task_completed_existing_run(
    db_session: AsyncSession,
) -> None:
    domain = await _seed_minimal_domain(db_session)
...
async def test_process_agent_task_completed_run_not_found(
    db_session: AsyncSession,
) -> None:
    result = await inbound_processors.process_agent_task_completed(
        db_session,
        {"run_id": 99999, "metrics": {}},
    )
    assert result["status"] == "ignored"
    assert result["reason"] == "run not found"


async def test_process_agent_escalation_duplicate(
    db_session: AsyncSession,
) -> None:
    payload = {
        "event_id": "esc-direct-dup",
        "agent_subject_id": 1,
        "reason": "x",
    }
    first = await inbound_processors.process_agent_escalation(
        db_session,
        payload,
    )
    assert first["status"] == "processed"

    second = await inbound_processors.process_agent_escalation(
        db_session,
        payload,
    )
...
async def test_process_council_synthesis_duplicate(
    db_session: AsyncSession,
) -> None:
    payload = {
        "event_id": "council-direct-dup",
        "council_id": "c1",
        "outcome": "ok",
    }
    first = await inbound_processors.process_council_synthesis(
        db_session,
        payload,
    )
    assert first["status"] == "processed"

    second = await inbound_processors.process_council_synthesis(
        db_session,
        payload,
    )
    assert second["status"] == "ignored"
...
async def test_models_persisted_after_processing(
    db_session: AsyncSession,
) -> None:
