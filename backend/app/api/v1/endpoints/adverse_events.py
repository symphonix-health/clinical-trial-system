...
async def create_adverse_event(
    data: schemas.AdverseEventCreate, db: AsyncSession = Depends(get_db)
) -> schemas.AdverseEventOut:
...
async def list_adverse_events(
    study_id: int | None = None, db: AsyncSession = Depends(get_db)
...
async def update_adverse_event(
    ae_id: int, data: schemas.AdverseEventUpdate, db: AsyncSession = Depends(get_db)
) -> schemas.AdverseEventOut:
    ae = await crud.get_adverse_event(db, ae_id)
    if not ae:
        raise HTTPException(status_code=404, detail="Adverse event not found")
