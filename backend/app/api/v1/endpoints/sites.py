...
@router.get(
    "/{site_id}/checklist",
    response_model=list[schemas.SiteActivationChecklistOut],
)
async def get_site_checklist(
    ...
    return [
        schemas.SiteActivationChecklistOut.model_validate(t)
        for t in tasks
    ]


@router.patch(
    "/{site_id}/checklist/{task_name}",
    response_model=schemas.SiteActivationChecklistOut,
)
async def update_checklist_task(
    ...
    task = await crud.update_checklist_task(
        db,
        site_id,
        task_name,
        status,
