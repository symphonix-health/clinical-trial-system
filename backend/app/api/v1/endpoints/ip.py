...
@router.get(
    "/products/{product_id}",
    response_model=schemas.InvestigationalProductOut,
)
...
@router.post(
    ...
    response_model=schemas.IpShipmentOut,
)
async def receive_shipment(
    ...
    updated = await crud.receive_shipment(
        db,
        shipment,
        received_by,
        condition_ok,
...
@router.post(
    "/dispenses/{dispense_id}/destroy",
    response_model=schemas.IpDispenseOut,
)
