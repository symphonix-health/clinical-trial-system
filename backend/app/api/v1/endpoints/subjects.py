...
async def list_subjects(
    study_id: int | None = None, site_id: int | None = None, db: AsyncSession = Depends(get_db)
...
async def update_subject(
    subject_id: int, data: schemas.SubjectUpdate, db: AsyncSession = Depends(get_db)
) -> schemas.SubjectOut:
    subject = await crud.get_subject(db, subject_id)
    if not subject:
        raise HTTPException(status_code=404, detail="Subject not found")
...
async def record_consent(
    subject_id: int, data: schemas.InformedConsentCreate, db: AsyncSession = Depends(get_db)
...
async def randomise_subject(
    subject_id: int, data: schemas.RandomiseSubject, db: AsyncSession = Depends(get_db)
) -> schemas.SubjectOut:
    ...
    if not subject:
        raise HTTPException(status_code=404, detail="Subject not found")
    updated, _allocation = await crud.randomise_subject(db, subject, data.stratification_factors, data.randomised_by)
    return schemas.SubjectOut.model_validate(updated)


@router.get("/{subject_id}/allocation", response_model=schemas.RandomisationResult)
async def get_allocation(subject_id: int, db: AsyncSession = Depends(get_db)) -> schemas.RandomisationResult:
    ...
    result = await db.execute(
        select(models.RandomisationAllocation).where(models.RandomisationAllocation.allocated_subject_id == subject_id)
    )
