...
@router.get(
    "/recruitment/{study_id}",
    response_model=schemas.RecruitmentReport,
)
async def recruitment_report(
    study_id: int,
    db: AsyncSession = Depends(get_db),
) -> schemas.RecruitmentReport:
    return await crud.recruitment_report(db, study_id)


@router.get(
    "/safety/{study_id}",
    response_model=schemas.SafetyReport,
)
async def safety_report(
    study_id: int,
    db: AsyncSession = Depends(get_db),
) -> schemas.SafetyReport:
    return await crud.safety_report(db, study_id)


@router.get(
    "/etmf/{study_id}",
    response_model=schemas.ETMFReport,
)
async def etmf_report(
    study_id: int,
    db: AsyncSession = Depends(get_db),
) -> schemas.ETMFReport:
...
@router.get(
    "/ip-accountability/{study_id}/{site_id}",
    response_model=schemas.AccountabilityReport,
)
