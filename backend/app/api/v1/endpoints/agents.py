...
async def list_agent_subjects(
    ...
    agents = await crud.list_agent_subjects(
        db,
        enrolled_study_id=enrolled_study_id,
    )
...
@router.post(
    "/subjects/{agent_id}/attestations",
    response_model=schemas.AgentAttestationOut,
)
...
@router.post(
    "/subjects/{agent_id}/consent-contracts",
    response_model=schemas.AgentConsentContractOut,
)
...
async def create_cohort(
    data: schemas.AgentCohortCreate,
    db: AsyncSession = Depends(get_db),
) -> schemas.AgentCohortOut:
...
async def add_to_cohort(
    ...
    membership = await crud.add_agent_to_cohort(
        db,
        cohort_id,
        agent_subject_id,
    )
...
@router.get(
    "/runs/{run_id}/metrics",
    response_model=list[schemas.AgentMetricOut],
)
async def list_run_metrics(
    run_id: int,
    db: AsyncSession = Depends(get_db),
) -> list[schemas.AgentMetricOut]:
...
@router.get(
    "/cohorts/{cohort_id}/release-gate",
    response_model=schemas.ReleaseGateResult,
)
