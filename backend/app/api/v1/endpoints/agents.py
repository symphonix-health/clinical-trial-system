...
async def create_agent_subject(
    data: schemas.AgentSubjectCreate, db: AsyncSession = Depends(get_db)
...
async def list_agent_subjects(
    enrolled_study_id: int | None = None, db: AsyncSession = Depends(get_db)
...
async def update_agent_subject(
    agent_id: int, data: schemas.AgentSubjectUpdate, db: AsyncSession = Depends(get_db)
) -> schemas.AgentSubjectOut:
    agent = await crud.get_agent_subject(db, agent_id)
    if not agent:
        raise HTTPException(status_code=404, detail="Agent subject not found")
...
async def create_attestation(
    agent_id: int, data: schemas.AgentAttestationCreate, db: AsyncSession = Depends(get_db)
...
async def create_environment(
    data: schemas.SyntheticEnvironmentCreate, db: AsyncSession = Depends(get_db)
...
async def complete_run(
    run_id: int, metrics: "dict[str, float]", trace_url: str | None = None, db: AsyncSession = Depends(get_db)
) -> schemas.AgentRunOut:
...
async def create_trial_arm(
    data: schemas.AgentTrialArmCreate, db: AsyncSession = Depends(get_db)
...
async def create_bias_report(
    data: schemas.AgentBiasReportCreate, db: AsyncSession = Depends(get_db)
